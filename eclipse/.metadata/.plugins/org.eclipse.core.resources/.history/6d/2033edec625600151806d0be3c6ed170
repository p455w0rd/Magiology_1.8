...
//		MEvents.EventRegister(0, new Helper());
	...
	public static double[] cricleXZ(double angle){
		...
		result[0]=CricleHelper.sin(intAngle);//-X-
		result[1]=CricleHelper.cos(intAngle);//-Z-
		return result;
	}
	...
	public static double[] createBallXYZ(double ballSize, boolean hasSecondPos){
		...
		result[0]=CricleHelper.sin(xRot)*CricleHelper.cos(yRot);//-X-
		result[1]=CricleHelper.sin(yRot);//-Y-
		result[2]=CricleHelper.cos(xRot)*CricleHelper.cos(yRot);//-Z-
		if(hasSecondPos){
			xRot+=CRandI(50);
			yRot+=CRandI(50);
			result[3]=CricleHelper.sin(xRot)*CricleHelper.cos(yRot);//-X-
			result[4]=CricleHelper.sin(yRot);//-Y-
			result[5]=CricleHelper.cos(xRot)*CricleHelper.cos(yRot);//-Z-
		}
