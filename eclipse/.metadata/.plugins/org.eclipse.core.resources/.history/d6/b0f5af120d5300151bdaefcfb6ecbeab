...
public class SpecialPlayerParicleHandeler_SubEvent{
	...
	SlowdownHelper slowdown=new SlowdownHelper(10);
	SlowdownHelper slowdown2=new SlowdownHelper(20);
	
	public void onUpdate(World world,EntityPlayer player,double x,double y,double z,double xv,double yv,double zv,int type){
		...
		if(isActive){
			if(player.capabilities.isCreativeMode){
				...
				if(world.getTotalWorldTime()%2==0){
					...
					Helper.spawnEntityFX(new EntityFollowingBubleFX(world,x+ab[0], y+0.3, z+ab[1], xv*1.2, yv*1.2, zv*1.2,player,angle, ab[0], 0.3, ab[1], 250, 12*(player.isSneaking()?2:1), world.rand.nextFloat(),world.rand.nextFloat(),world.rand.nextFloat(),0.5));
				}
			}
			...
			if((player.isOnLadder()&&Math.round((player.motionY+0.06)*10)!=0)){
				...
				for (int i=0;i<ammount;i++){
					...
					Helper.spawnEntityFX(new EntitySmoothBubleFX(world,xr,yr,zr, xv*1.1,yv*1.1,zv*1.1, size, 2+world.rand.nextInt(2), 0,1, R,G,B, 1, 0.98));
				}
			}
...
//				System.out.print("\n"+BB);
			for (int i=0;i<ammount;i++){
				...
				if((player.isSneaking()?world.rand.nextBoolean():true))Helper.spawnEntityFX(new EntitySmoothBubleFX(world,xr,yr,zr, xv*0.5+Helper.CRandF(0.05),yv*0.5+Helper.CRandF(0.05),zv*0.5+Helper.CRandF(0.05), size*(player.isSneaking()?2:1), 2+world.rand.nextInt(2), player.isSneaking()?1:6,true,player.isSneaking()?2:1,"tx1", R,G,B, 1, 0.98));
			}
			
		}
	}
	
	public void onJump(World world,EntityPlayer player,double x,double y,double z,double xv,double yv,double zv,int type){
		if(isActive){
			...
			for(int i=0;i<20*ammount;i++){
				xr=x+Helper.CRandF(0.8);
				yr=y-1.5;
				zr=z+Helper.CRandF(0.8);
				scale=world.rand.nextDouble()*0.4;
				R=world.rand.nextFloat()/3;G=world.rand.nextFloat()/3;B=1;
				if(itemS!=null&&(itemS.getItem()==MItems.FireHammer||Block.getBlockFromItem(itemS.getItem())==MBlocks.FirePipe)){
					R=1;G=world.rand.nextFloat()/3;B=world.rand.nextFloat()/3;
				}
				Helper.spawnEntityFX(new EntitySmoothBubleFX(world,xr, yr, zr,  Helper.CRandF(0.1+scale), Helper.CRandF(0.1+scale)+0.5, Helper.CRandF(0.1+scale), (int)(300+world.rand.nextInt(300)*(player.isSneaking()?0.1:1)), 3+world.rand.nextInt(2), -20,true,player.isSneaking()?2:1,"tx1", R,G,B, 1, 0.95));
			}
			if(ammount!=1)for(int a=0;a<ammount*2;a++){
				...
				Helper.spawnEntityFX(new EntitySmoothBubleFX(world,xr, yr, zr,  Helper.CRandF(0.1+scale), Helper.CRandF(0.4+scale)+1, Helper.CRandF(0.1+scale), 300+world.rand.nextInt(300), 3+world.rand.nextInt(2), -20,1, R,G,B, 1, 1));
			}
		}
	}
	...
	public void onHitTheGround(World world,double x,double y,double z,EntityPlayer player, float distance){
		if(isActive){
			...
			if(distance>3)for(int a=0;a<4*(int)distance;a++){
				...
				zr=z+Helper.CRandF(0.8);
				scale=world.rand.nextDouble()*0.4;
				Helper.spawnEntityFX(new EntitySmoothBubleFX(world,xr, yr, zr,  Helper.CRandF(0.1+scale), Helper.CRandF(0.1+scale), Helper.CRandF(0.1+scale), 300+world.rand.nextInt(300), 3+world.rand.nextInt(2), -20,1, world.rand.nextFloat(),world.rand.nextFloat(),world.rand.nextFloat(), 1, 0.95));
