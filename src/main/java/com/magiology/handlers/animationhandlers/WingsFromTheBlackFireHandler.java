...
public class WingsFromTheBlackFireHandler{
	...
	public static Positions[] getFlap(EntityPlayer player){
		...
		if(!player.worldObj.isRemote&&timer==0){
			...
			if(mainFlap==Positions.FlapDownPos&&(id==Positions.FlyForvardPos||id==Positions.FlyStationarPos||id==Positions.ErrorPos))UtilM.playSoundAtEntity(Sounds.WingSwingFX, player, 0.3+(y!=0?(y==1?0.2:-0.1):0), 1);
		}
