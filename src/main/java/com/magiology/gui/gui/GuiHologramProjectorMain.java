...
import com.magiology.gui.custom.OnOffGuiButton;
...
public class GuiHologramProjectorMain extends GuiContainer implements Updateable{
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY){
		...
		if(helpAlpha<0.97){
			...
			GL11.glTranslatef((-Get.Render.FR().getStringWidth(s)/4+xSize)/2, 0, 0);
			Get.Render.FR().drawStringWithShadow(s, 0, 0, new ColorF(1,1,1,0.5F*(1-helpAlpha)).toCode());
			GL11.glPopMatrix();
		}
		if(helpAlpha>0.03){
			int color=new ColorF(1, 1, 1, helpAlpha).toCode();
			String ish="Is highlight™ going to be be applied";
			Get.Render.FR().drawStringWithShadow(ish, 150-Get.Render.FR().getStringWidth(ish), ySize+2, color);
			Get.Render.FR().drawStringWithShadow("Spawn new componenet", -10, -9, color);
