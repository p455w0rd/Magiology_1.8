...
public class ModInfoGUI extends JFrame{
		...
		if(command.startsWith("c")||command.startsWith("C")){
			...
			if(chars.length>=2){
				if(chars[0]=='C'||chars[0]=='c'){
					...
					if(Helper.isInteger(stringedNumber)){
						isValid=true;
	...
	public void useCommand(String command){
		if(command.equals("activate MLG mode!")){
			for(int a=0;a<30;a++)addLine(newLine("nope...",new Font(Font.SANS_SERIF, Font.BOLD,15), new Color(Helper.RInt(255),Helper.RInt(255),Helper.RInt(255)), true));
			Helper.throwException(new NopeException());
			return;
		}
		addLine(newLine("Command on "+commandID+(command.equals("#autoInvoked")?" automatically":"")+" invoked!",new Font(Font.SANS_SERIF, Font.BOLD,15), new Color(Helper.RInt(255),Helper.RInt(255),Helper.RInt(255)), true));
		...
		case 1:{
			isInputInvoked=true;
			int res=-1;
			if(Helper.isInteger(command))res=Integer.parseInt(command);
			else if(Helper.isBoolean(command))res=Helper.booleanToInt(Boolean.parseBoolean(command));
			if(res==0){
				autoRemove=true;
				isInputOk=true;
		...
		if(!isInputOk&&!isInputInvoked)addLine(newLine("NoSuchCommand exception!",new Font(Font.SANS_SERIF, Font.BOLD,15), new Color(Helper.RInt(255),Helper.RInt(255),Helper.RInt(255)), true));
		else if(!isInputOk&&isInputInvoked)addLine(newLine("WrongInput exception!",new Font(Font.SANS_SERIF, Font.BOLD,15), new Color(Helper.RInt(255),Helper.RInt(255),Helper.RInt(255)), true));
		item1.setText("");
		Update();
	}
	...
	public Line newLine(String string,Font font,Color color){return this.new Line(string, font,color,false);}
	public Line newLine(String string,Font font,Color color,boolean isUserCasted){return this.new Line(string, font,color,isUserCasted);}
	public Line newLine(String string,Color color,boolean isUserCasted){return this.new Line(string, new Font(Font.SANS_SERIF, Font.PLAIN,15),color,isUserCasted);}
	public Line newLine(String string,Color color){return this.new Line(string, new Font(Font.SANS_SERIF, Font.PLAIN,15),color,false);}
	public class Line{
