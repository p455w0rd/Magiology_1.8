...
public class SpecialPlayerParicleHandeler{
		...
		if(isActive){
			if(player.capabilities.isCreativeMode){
				...
				if(world.getTotalWorldTime()%2==0){
					...
					Util.spawnEntityFX(new EntityFollowingBubleFX(world,x+ab[0], y+0.3, z+ab[1], xv*1.2, yv*1.2, zv*1.2,player,angle, ab[0], 0.3, ab[1], 250, 12*(player.isSneaking()?2:1), world.rand.nextFloat(),world.rand.nextFloat(),world.rand.nextFloat(),0.5));
				}
			}
			...
			if((player.isOnLadder()&&Math.round((player.motionY+0.06)*10)!=0)){
				...
				for (int i=0;i<ammount;i++){
					double xr=x+Util.CRandF(0.8);
					double yr=y+Util.CRandF(1.8)-0.6;
					double zr=z+Util.CRandF(0.8);
					...
					Util.spawnEntityFX(new EntitySmoothBubleFX(world,xr,yr,zr, xv*1.1,yv*1.1,zv*1.1, size, 2+world.rand.nextInt(2), 0,1, R,G,B, 1, 0.98));
				}
			}
			...
			if(slowdown2.isTimeWithAddProgress())for(int a=0;a<ammount;a++){
				...
				do{
					...
					block=Util.getBlock(world, xP, yP, zP);
				}while(block!=Blocks.air&&count<100);
				Util.spawnEntityFX(new EntitySmoothBubleFX(world,xP+0.5, yP+1.5, zP+0.5,0.04*(world.rand.nextBoolean()?1:-1), 0.04*(world.rand.nextBoolean()?1:-1), 0.04*(world.rand.nextBoolean()?1:-1),600,0,0,true,2,"tx1",world.rand.nextFloat(),world.rand.nextFloat(),world.rand.nextFloat(),1, 0.99));
...
//				System.out.print("\n"+BB);
			for (int i=0;i<ammount;i++){
				...
				if((player.isSneaking()?world.rand.nextBoolean():true))Util.spawnEntityFX(new EntitySmoothBubleFX(world,xr,yr,zr, xv*0.5+Util.CRandF(0.05),yv*0.5+Util.CRandF(0.05),zv*0.5+Util.CRandF(0.05), size*(player.isSneaking()?2:1), 2+world.rand.nextInt(2), player.isSneaking()?1:6,true,player.isSneaking()?2:1,"tx1", R,G,B, 1, 0.98));
			}
	...
	public void onJump(World world,EntityPlayer player,double x,double y,double z){
		if(isActive){
			...
			for(int i=0;i<20*ammount;i++){
				xr=x+Util.CRandF(0.8);
				yr=y-1.5;
				zr=z+Util.CRandF(0.8);
				scale=world.rand.nextDouble()*0.4;
				R=world.rand.nextFloat()/3;G=world.rand.nextFloat()/3;B=1;
				if(itemS!=null&&(itemS.getItem()==MItems.FireHammer||Block.getBlockFromItem(itemS.getItem())==MBlocks.FirePipe)){
					R=1;G=world.rand.nextFloat()/3;B=world.rand.nextFloat()/3;
				}
				Util.spawnEntityFX(new EntitySmoothBubleFX(world,xr, yr, zr,  Util.CRandF(0.1+scale), Util.CRandF(0.1+scale)+0.5, Util.CRandF(0.1+scale), (int)(300+world.rand.nextInt(300)*(player.isSneaking()?0.1:1)), 3+world.rand.nextInt(2), -20,true,player.isSneaking()?2:1,"tx1", R,G,B, 1, 0.95));
			}
			if(ammount!=1)for(int a=0;a<ammount*2;a++){
				...
				Util.spawnEntityFX(new EntitySmoothBubleFX(world,xr, yr, zr,  Util.CRandF(0.1+scale), Util.CRandF(0.4+scale)+1, Util.CRandF(0.1+scale), 300+world.rand.nextInt(300), 3+world.rand.nextInt(2), -20,1, R,G,B, 1, 1));
			}
		}
	}
	...
	public void onHitTheGround(World world,double x,double y,double z,EntityPlayer player, float distance){
		if(isActive){
			...
			if(distance>3)for(int a=0;a<4*(int)distance;a++){
				...
				zr=z+Util.CRandF(0.8);
				scale=world.rand.nextDouble()*0.4;
				Util.spawnEntityFX(new EntitySmoothBubleFX(world,xr, yr, zr,  Util.CRandF(0.1+scale), Util.CRandF(0.1+scale), Util.CRandF(0.1+scale), 300+world.rand.nextInt(300), 3+world.rand.nextInt(2), -20,1, world.rand.nextFloat(),world.rand.nextFloat(),world.rand.nextFloat(), 1, 0.95));
