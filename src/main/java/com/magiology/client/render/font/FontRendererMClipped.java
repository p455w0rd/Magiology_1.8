package com.magiology.client.render.font;

import java.awt.Rectangle;

import org.lwjgl.util.vector.Matrix4f;
import org.lwjgl.util.vector.Vector2f;

import com.magiology.util.renderers.VertexRenderer;
import com.magiology.util.utilclasses.Get;
import com.magiology.util.utilclasses.math.MatrixUtil;
import com.magiology.util.utilobjects.vectors.Vec3M;

import net.minecraft.util.ResourceLocation;


public class FontRendererMClipped extends FontRendererMBase{
	
	protected static FontRendererMClipped instance=new FontRendererMClipped(new ResourceLocation("textures/font/ascii.png")); 
	public static FontRendererMClipped get(){
		return instance;
	}
	protected static VertexRenderer rednerer=Get.Render.NVB();
	public Vector2f min=new Vector2f(),max=new Vector2f();
	
	public FontRendererMClipped(ResourceLocation res){
		super(res);
	}
	
	
	/**
	 * Render a single line string at the current (posX,posY) and update posX
	 */
	@Override
	protected void renderStringAtPos(String p_78255_1_, boolean p_78255_2_){

		
		for(int i=0;i<p_78255_1_.length();++i){
			char c0=p_78255_1_.charAt(i);
			int j,k;
			if(c0==167&&i+1<p_78255_1_.length()){
				j="0123456789abcdefklmnor".indexOf(p_78255_1_.toLowerCase().charAt(i+1));
				if(j<16){
					randomStyle=false;
					boldStyle=false;
					strikethroughStyle=false;
					underlineStyle=false;
					italicStyle=false;
					if(j<0||j>15){
						j=15;
					}
					if(p_78255_2_){
						j+=16;
					}
					k=colorCode[j];
					textColor=k;
					setColor((k>>16)/255.0F, (k>>8&255)/255.0F, (k&255)/255.0F, color.a);
				}else if(j==16)randomStyle=true;
				else if(j==17)boldStyle=true;
				else if(j==18)strikethroughStyle=true;
				else if(j==19)underlineStyle=true;
				else if(j==20)italicStyle=true;
				else if(j==21){
					randomStyle=false;
					boldStyle=false;
					strikethroughStyle=false;
					underlineStyle=false;
					italicStyle=false;
					color.bind();
				}
				++i;
			}else{
				j="\u00c0\u00c1\u00c2\u00c8\u00ca\u00cb\u00cd\u00d3\u00d4\u00d5\u00da\u00df\u00e3\u00f5\u011f\u0130\u0131\u0152\u0153\u015e\u015f\u0174\u0175\u017e\u0207\u0000\u0000\u0000\u0000\u0000\u0000\u0000 !\"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~\u0000\u00c7\u00fc\u00e9\u00e2\u00e4\u00e0\u00e5\u00e7\u00ea\u00eb\u00e8\u00ef\u00ee\u00ec\u00c4\u00c5\u00c9\u00e6\u00c6\u00f4\u00f6\u00f2\u00fb\u00f9\u00ff\u00d6\u00dc\u00f8\u00a3\u00d8\u00d7\u0192\u00e1\u00ed\u00f3\u00fa\u00f1\u00d1\u00aa\u00ba\u00bf\u00ae\u00ac\u00bd\u00bc\u00a1\u00ab\u00bb\u2591\u2592\u2593\u2502\u2524\u2561\u2562\u2556\u2555\u2563\u2551\u2557\u255d\u255c\u255b\u2510\u2514\u2534\u252c\u251c\u2500\u253c\u255e\u255f\u255a\u2554\u2569\u2566\u2560\u2550\u256c\u2567\u2568\u2564\u2565\u2559\u2558\u2552\u2553\u256b\u256a\u2518\u250c\u2588\u2584\u258c\u2590\u2580\u03b1\u03b2\u0393\u03c0\u03a3\u03c3\u03bc\u03c4\u03a6\u0398\u03a9\u03b4\u221e\u2205\u2208\u2229\u2261\u00b1\u2265\u2264\u2320\u2321\u00f7\u2248\u00b0\u2219\u00b7\u221a\u207f\u00b2\u25a0\u0000".indexOf(c0);
				if(randomStyle&&j!=-1){
					do{k=fontRandom.nextInt(charWidth.length);}while(charWidth[j]!=charWidth[k]);
					j=k;
				}
				float f1=getCharWidth(c0)/32f;
				boolean flag1=(c0==0||j==-1||getUnicodeFlag())&&p_78255_2_;
				if(flag1){posX-=f1;posY-=f1;}
				float f=renderCharAtPos(j, c0, italicStyle);
				if(flag1){posX+=f1;posY+=f1;}
				if(boldStyle){
					posX+=f1;
					if(flag1){posX-=f1;posY-=f1;}
					renderCharAtPos(j, c0, italicStyle);
					posX-=f1;
					if(flag1){posX+=f1;posY+=f1;}
					++f;
				}
				doDraw(f);
			}
		}
		bindTexture(locationFontTexture);
		rednerer.setInstantNormalCalculation(false);
		rednerer.draw();
		rednerer.setInstantNormalCalculation(true);
		min=new Vector2f();
		max=new Vector2f();
	}
	/**
	 * Render a single character with the default.png font at current
	 * (posX,posY) location...
	 */
	@Override
	protected float renderDefaultChar(int p_78266_1_, boolean p_78266_2_){
		float f=p_78266_1_%16*8,f1=p_78266_1_/16*8,f2=p_78266_2_?1.0F:0.0F,f3=charWidth[p_78266_1_]-0.01F;
		Vector2f zero=new Vector2f();
		if(!min.equals(zero)&&!max.equals(zero)){
			Rectangle clip=new Rectangle((int)min.x, (int)min.y, (int)(max.x-min.x), (int)(max.y-min.y));
			Vector2f min=new Vector2f(posX+f2, posY),max=new Vector2f(posX+f3-1.0F-f2, posY+7.99F);
			Matrix4f transf=rednerer.getTransformation();
			Vec3M Min=MatrixUtil.transformVector(new Vec3M(min.x, min.y, 0), transf),Max=MatrixUtil.transformVector(new Vec3M(max.x, max.y, 0), transf);
			if(clip.contains(Min.x,Min.y)||clip.contains(Max.x, Max.y)){
				Vector2f push=new Vector2f();
				if(Min.x<this.min.x)min.x+=push.x=(float)(this.min.x-Min.x);
				else if(Max.x>this.max.x)max.x+=push.x=(float)(this.max.x-Max.x);
				if(Min.y<this.min.y)min.y+=push.y=(float)(this.min.y-Min.y);
				else if(Max.y>this.max.y)max.y+=push.y=(float)(this.max.y-Max.y);
				push.x/=charWidth[p_78266_1_];
				push.y/=8F;
				float 
					minXUV=(f+Math.max(push.x*charWidth[p_78266_1_], 0))/128.0F,maxXUV=(f+f3-1.0F+Math.min(push.x*charWidth[p_78266_1_], 0))/128.0F,
					minYUV=(f1+Math.max((push.y-0.006F)*FONT_HEIGHT, 0))/128.0F,maxYUV=((f1+Math.min(push.y*FONT_HEIGHT, 0))+8F)/128.0F;
				rednerer.addVertexWithUV(min.x, min.y, 0.0F, minXUV, minYUV);
				rednerer.addVertexWithUV(min.x, max.y, 0.0F, minXUV, maxYUV);
				rednerer.addVertexWithUV(max.x, max.y, 0.0F, maxXUV, maxYUV);
				rednerer.addVertexWithUV(max.x, min.y, 0.0F, maxXUV, minYUV);
			}
		}else{
			Vector2f min=new Vector2f(posX+f2, posY),max=new Vector2f(posX+f3-1.0F-f2, posY+7.99F);
			float minXUV=f/128.0F,maxXUV=(f+f3-1.0F)/128.0F,minYUV=f1/128.0F,maxYUV=(f1+7.99F)/128.0F;
			rednerer.addVertexWithUV(min.x, min.y, 0.0F, minXUV, minYUV);
			rednerer.addVertexWithUV(min.x, max.y, 0.0F, minXUV, maxYUV);
			rednerer.addVertexWithUV(max.x, max.y, 0.0F, maxXUV, maxYUV);
			rednerer.addVertexWithUV(max.x, min.y, 0.0F, maxXUV, minYUV);
		}
		return charWidth[p_78266_1_];
	}
	
}
