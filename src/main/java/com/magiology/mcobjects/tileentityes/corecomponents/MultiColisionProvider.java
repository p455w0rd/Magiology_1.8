...
public interface MultiColisionProvider{
	...
	public class MultiColisionProviderRayTracer{
		...
		public static int getRayTracedBoxId(World world, int x, int y, int z, Vec3 startVec, Vec3 endVec,AxisAlignedBB resetBoundsOptional){
			TileEntity tester=world.getTileEntity(x, y, z);
			if(!(tester instanceof MultiColisionProvider)){
				Helper.println("There is no instance of ISidedColisionProvider at"+"("+x+", "+y+", "+z+")!",Helper.getStackTrace());
				return -1;
			}
			...
			for(int i=0;i<aciveBoxes.length;i++){
				block.setBlockBounds((float)aciveBoxes[i].minX,(float)aciveBoxes[i].minY,(float)aciveBoxes[i].minZ,(float)aciveBoxes[i].maxX,(float)aciveBoxes[i].maxY,(float)aciveBoxes[i].maxZ);
	       		results=ArrayUtils.add(results, block.collisionRayTrace(world, x, y, z, startVec, endVec));
			...
			if(results.length>0){
				for(int id1=0;id1<results.length;id1++){
					...
					if(result1!=null&&result1.hitVec!=null){
						double distance=startVec.distanceTo(result1.hitVec);
