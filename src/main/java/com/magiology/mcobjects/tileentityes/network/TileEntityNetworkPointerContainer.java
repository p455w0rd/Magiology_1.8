...
public class TileEntityNetworkPointerContainer extends TileEntityNetwork implements ISidedInventory{
	...
	public void setColisionBoxes(){
		...
		collisionBoxes=new AxisAlignedBB[]{
				connections[5]!=null?Helper.AxisAlignedBB(0,   p*6, p*6, p*5,  p*10, p*10):null,
				connections[1]!=null?Helper.AxisAlignedBB(p*6, 0,   p*6, p*10, p*5,  p*10):null,
				connections[2]!=null?Helper.AxisAlignedBB(p*6, p*6, 0,   p*10, p*10, p*5 ):null,
				connections[3]!=null?Helper.AxisAlignedBB(p*11,p*6, p*6, 1,    p*10, p*10):null,
				connections[0]!=null?Helper.AxisAlignedBB(p*6, p*11,p*6, p*10, 1,    p*10):null,
				connections[4]!=null?Helper.AxisAlignedBB(p*6, p*6, p*11,p*10, p*10, 1   ):null,
						Helper.AxisAlignedBB(p*5, p*5, p*5, p*11, p*11, p*11),
		};
//		Helper.printInln(side);
			...
			for(int j=0;j<3;j++){
				...
				case 4:{
					collisionBoxes=ArrayUtils.add(collisionBoxes,
							Helper.AxisAlignedBB(p*9.5-i*p*2, p*9.5-j*p*2, p*4.8, p*10.5-i*p*2, p*10.5-j*p*2, p*10));
				}break;
				case 2:{
					collisionBoxes=ArrayUtils.add(collisionBoxes,
							Helper.AxisAlignedBB(p*9.5-i*p*2, p*9.5-j*p*2, p*6, p*10.5-i*p*2, p*10.5-j*p*2, p*11.2));
				}break;
				case 3:{
					collisionBoxes=ArrayUtils.add(collisionBoxes,
							Helper.AxisAlignedBB(p*4.8, p*9.5-j*p*2, p*9.5-i*p*2, p*10, p*10.5-j*p*2, p*10.5-i*p*2));
				}break;
				case 5:{
					collisionBoxes=ArrayUtils.add(collisionBoxes,
							Helper.AxisAlignedBB(p*6, p*9.5-j*p*2, p*9.5-i*p*2, p*11.2, p*10.5-j*p*2, p*10.5-i*p*2));
				}break;
				case 0:{
					collisionBoxes=ArrayUtils.add(collisionBoxes,
							Helper.AxisAlignedBB(p*9.5-i*p*2, p*4.8, p*9.5-j*p*2, p*10.5-i*p*2, p*10, p*10.5-j*p*2));
				}break;
				case 1:{
					collisionBoxes=ArrayUtils.add(collisionBoxes,
							Helper.AxisAlignedBB(p*9.5-i*p*2, p*6, p*9.5-j*p*2, p*10.5-i*p*2, p*11.2, p*10.5-j*p*2));
				}break;
				}
			}
		}
	...
	public NetworkBaseInterface getBoundedBaseInterface(){
		int side=SideHelper.convert(getOrientation());
		TileEntity test=worldObj.getTileEntity(SideHelper.X(side, xCoord), SideHelper.Y(side, yCoord), SideHelper.Z(side, zCoord));
