...
public class TileEntityNetworkController extends TileEntityNetworkPow{
	...
	public TileEntityNetworkController(){
		super(SixSidedBoolean.create(
				...
				SideHelper.ForgeDirgetOrientationInverted(ForgeDirection.UP),SideHelper.ForgeDirgetOrientationInverted(ForgeDirection.DOWN),
	...
	public void setColisionBoxes(){
		...
		collisionBoxes[0]=connections[5]!=null?Helper.AxisAlignedBB(p*6-p*6,p*6,p*6,p*6,p*10,p*10):null;
		collisionBoxes[1]=connections[1]!=null?Helper.AxisAlignedBB(p*6,p*6-p*6,p*6,p*10,p*6,p*10):null;
		collisionBoxes[2]=connections[2]!=null?Helper.AxisAlignedBB(p*6,p*6,p*6-p*6,p*10,p*10,p*6):null;
		collisionBoxes[3]=connections[3]!=null?Helper.AxisAlignedBB(p*10,p*6,p*6,p*10+p*6,p*10,p*10):null;
		collisionBoxes[4]=connections[0]!=null?Helper.AxisAlignedBB(p*6,p*10,p*6,p*10,p*10+p*6,p*10):null;
		collisionBoxes[5]=connections[4]!=null?Helper.AxisAlignedBB(p*6,p*6,p*10,p*10,p*10,p*10+p*6):null;
			...
			do{
				...
				for(int j=0;j<network.size();j++){
					...
					if(!isDone)for(int i=0;i<6;i++)if(workTile.getAccessibleOnSide(i)){
						TileEntity test=workTile.getHost().getWorldObj().getTileEntity(SideHelper.X(i, workTile.getHost().xCoord), SideHelper.Y(i, workTile.getHost().yCoord), SideHelper.Z(i, workTile.getHost().zCoord));
