...
public class WingsFromTheBlackFireHandeler{
	...
	public static enum Positions{
		...
		FlapForwardPos(false,counter.getAndAdd(),new float[][]{{0,4,5},{0,5,6},{0,7,7},{0,2,8},{0,3,2},{0,1,2},{0,1,2}}),
		FlapBackwardsPos(false,counter.getAndAdd(),new float[][]{{0,-3,-5},{0,-2,-6},{0,-3,-7},{0,-2,-8},{0,-3,-2},{0,-1,-2},{0,-1,-2}}),
	...
	public static Positions[] getFlap(EntityPlayer player){
		...
		if(!player.worldObj.isRemote&&timer==0){
			...
			if(mainFlap==Positions.FlapDownPos&&(id==Positions.FlyForvardPos||id==Positions.FlyStationarPos||id==Positions.FlyBackvardPos))Helper.playSoundAtEntity(Sounds.WingSwingFX, player, 0.3+(y!=0?(y==1?0.2:-0.1):0), 1);
		}
